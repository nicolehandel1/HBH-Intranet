	...
	// event types
		...
		function event_type_custom_columns($value, $column_name, $id){
			if($column_name == 'event_type_id'){
				...
				?>
				<span class="term_id"><?php echo $t_id?></span><?php 

				if( !empty($term_meta['et_color'] ) )
					echo '<span class="evoterm_color" style="background-color:#'.$term_meta['et_color'].'"></span>';

				if( !empty($term_meta['et_icon']) ) 
					echo '<span class="evoterm_icon"><i class="fa '.$term_meta['et_icon'].'"></i></span>';
			...
			function eventon_taxonomy_edit_meta_field($term) {
				...
				<tr class="form-field">
					...
					<td class='evo_metafield_image'>
						...
						<span class='evo_loc_image_src image_src'><img src='<?php echo $img_url? $img_url[0]:'';?>' style='<?php echo !empty($term_meta['evo_loc_img'])?'':'display:none';?>'/></span>
