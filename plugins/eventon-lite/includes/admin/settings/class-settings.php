	...
	function evo_save_settings(){
		...
		if( isset($_POST['evcal_noncename']) && isset( $_POST ) ){
			if ( wp_verify_nonce( $_POST['evcal_noncename'], AJDE_EVCAL_BASENAME ) ){
				...
				if($focus_tab=='evcal_1' && isset($post_data['evcal_css_head']) && $post_data['evcal_css_head']=='yes'){
				...
				// STYLES
				if( isset($post_data['evcal_styles']) )
					update_option('evcal_styles', strip_tags(stripslashes(  $post_data['evcal_styles'] )) );
			...
			foreach($cpav['fields'] as $field){
				...
				switch ($field['type']){
					...
					case 'yesno':						
						...
						$rightside.= "<p class='yesno_row'>". EVO()->elements->yesno_btn(array('var'=>$__default,'attr'=>array('afterstatement'=>$after_statement) ))."<input type='hidden' name='".$field['id']."' value='".(($__default=='yes')?'yes':'no')."'/><span class='field_name'>".__($field['name'],$textdomain)."{$legend_code}</span>";
