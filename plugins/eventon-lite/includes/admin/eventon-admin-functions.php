...
// get converted unix time for saving event date time using $_POST
	function evoadmin_get_unix_time_fromt_post($post_id=''){
		...
		// post values conversion
			...
			foreach($convert_data as $ff=>$vv){
				if(!isset( $postdata[ $ff ])) continue;

				$postdata[ $vv ] = $postdata[ $ff ];
			}
		...
		// DATE and TIME data
			...
			foreach($fields_sub_ar as $ff){
				...
				if(empty($postdata[$ff])) continue;
				$date_POST_values[$ff]=$postdata[$ff];
		...
		// hide end time filtering of data values
			if( !empty($postdata['evo_hide_endtime']) && $postdata['evo_hide_endtime']=='yes'){

				if(evo_settings_check_yn($postdata,'evo_span_hidden_end')){
					$date_POST_values['evcal_end_date']= $postdata['evcal_end_date'];
				}else{
					$date_POST_values['evcal_end_date']= $postdata['evcal_start_date'];
		...
		// convert the post times into proper unix time stamps
			$date_format = !empty($postdata['_evo_date_format']) ? $postdata['_evo_date_format']: get_option('date_format');
			$time_format = !empty($postdata['_evo_time_format']) ? $postdata['_evo_time_format']: get_option('time_format');
