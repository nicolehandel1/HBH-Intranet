			...
			switch ($column) {
				...
				case "event_type_2" :		
					if ( ! $terms = get_the_terms( $post->ID, $column ) ) {
						...
						foreach ( $terms as $term ) {
							$termlist[] = '<a href="' . admin_url( 'edit.php?' . $column . '=' . $term->slug . '&post_type=ajde_events' ) . ' ">' . esc_html( $term->name ). '</a>';
						}
	...
	// Duplicate event
		...
		function eventon_duplicate_event_post_button() {
			...
			if ( isset( $_GET['post'] ) ) {
				...
				$notifyUrl = wp_nonce_url( admin_url( "admin.php?action=duplicate_event&post=" . absint( $post_id ) ), 'eventon-duplicate-event_' . $post_id );
				?>
