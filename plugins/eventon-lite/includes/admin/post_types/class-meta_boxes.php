...
class evo_event_metaboxes{
	...
	// INIT meta boxes
		function metabox_init(){
			...
			$post_type = !empty($_GET['post_type'])? $_GET['post_type']: false;	   
	   		...
	   		if( $post_type != 'ajde_events' ) return false;
	...
	// $_POST is present
		function get_tax_form(){
			...
			$tax = isset($post_data['tax']) ? sanitize_text_field($post_data['tax']): false;
			...
			echo "<div class='evo_tax_entry' data-eventid='{$event_id}' data-tax='{$post_data['tax']}' data-type='{$post_data['type']}'>";
