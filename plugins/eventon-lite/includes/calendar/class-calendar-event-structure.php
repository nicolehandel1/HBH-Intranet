		...
		foreach($array as $element =>$elm){
			...
			switch($element){
				...
				case 'titles':
					...
					$OT.= "<span class='evcal_desc evo_info ". ( $year_long?'yrl ':null).( $month_long?'mnl ':null)."' {$event_location_variables} >";
					...
					$OT.= "<span class='evcal_desc2 evcal_event_title' itemprop='name'>". apply_filters('eventon_eventtop_maintitle',$EVENT->get_title() ) ."</span>";
			...
			switch($box_name){
				...
				// TIME
					case 'time':
						...
						$evc_time_text = "<span class='evo_eventcard_time_t'>". apply_filters('evo_eventcard_time', $object->timetext, $object) . "</span>";
