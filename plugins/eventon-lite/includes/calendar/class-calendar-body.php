...
class evo_cal_body{
	...
	// Calendar Header
		function get_calendar_header($arguments=''){
			...
			echo "<div id='evcal_calendar_". esc_html( $cal_id )."' class='". esc_attr( $_cal_classes_string )."' >";
				...
				if(!$external){
					...
					if($args['layout_changer'] =='yes'){
						echo "<p class='evo_layout_changer'>
						<i data-type='row' class='fa fa-bars'></i>
						<i data-type='bar' class='fa fa-grid-lines'></i>
					...
					// HTML 
						echo "<div id='evcal_head' class='calendar_header' >";
					...
					// if the calendar arrows and headers are to show 
						if($date_header){
							...
							echo "<p id='evcal_cur' class='evo_month_title'> ". esc_html($cal_header_title )."</p>";	
							// arrows
							...
							if(!empty($header_title)) 
								echo "<p class='evo_cal_other_header'>". esc_html($header_title) ."</p>";
						}
				...
				$class_string = esc_attr( implode(' ', apply_filters('eventon_events_list_classnames', $evcal_list_classes, $args)) .' '.$_classes_evcal_list );
		...
		public function cal_above_header($args){
			...
			if($jumper =='yes'){
				...
				$jumper_content.= "<div class='evo_j_container' style='display:".($exp_jumper=='yes'?'block':'none')."'>
					...
					foreach($fullMonther as $m){
						...
						$jumper_content.= "<a data-val='{$count}' {$_current} title='". esc_attr( $monthNAME).	"' >{$monthNAME}</a>";
						$count ++;
					}
			...
			// above calendar buttons
				...
				if(count($above_head)>0){
					$OUTPUT .= "<div class='evo_cal_above'>";
						foreach($above_head as $ff=>$v){

							if($ff=='evo-gototoday-btn'){
								$OUTPUT .= "<span class='".$ff."' style='display:none' data-mo='{$focused_month_num}' data-yr='{$focused_year}' data-dy=''>".$v."</span>";
							}else{
								$OUTPUT .= "<span class='". esc_attr( $ff )."'>". esc_html($v)."</span>";
							}							
						}
						
						$OUTPUT .= apply_filters('evo_cal_above_header_btns_end', $args);
				...
				if(count($above_heade_content)>0){
					$OUTPUT .= "<div class='evo_cal_above_content'>";
