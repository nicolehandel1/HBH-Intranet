...
class EVO_Cal_Filering{
	public function __construct(){
		...
		add_filter('evo_cal_above_header_btns_end', array($this,'sort_icon'), 10,1);
	}
	...
	function sort_icon($args){
		...
		$OUTPUT .= "<span class='evo-sort-btn'>";
		...
		if( $this->cal->evcal_hide_sort != 'yes'){ // if sort bar is set to show
			...
			if(count($sorting_options)>0){
				$OUTPUT .= "<div class='eventon_sort_line' style='display:none'>";

					$cnt =1;
					foreach($evsa1 as $so=>$sov){
						if(in_array($so, $sorting_options) || $so=='date' ){
						$OUTPUT .= "<p data-val='sort_".$so."' data-type='".$so."' class='evs_btn ".( ($args['sort_by'] == $sort_options[$cnt])? 'evs_hide':null)."' >"
